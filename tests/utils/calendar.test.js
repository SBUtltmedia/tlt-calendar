...
describe('calendar', () => {
  ...
  it('places an item (max = 2)', () => {
    ...
    const item2 = {value: 2, day: 0, hour: 0, minute: 30, duration: HALF_HOUR};
    const item3 = {value: 3, day: 0, hour: 1, minute: 0, duration: HALF_HOUR};
    expect(placeItem({}, item2, {maxItems})).to.deep.equal({"30": [item2]});
    expect(placeItem({"30": [item2], "60": [item3]}, item1, {maxItems})).to.deep.equal({"0": [item1], "30": [item2], "60": [item3]});
  });
  ...
  it('places an item (override multiples, full slot)', () => {
    ...
    expect(placeItem({"30": [item2], "60": [item3]}, item5, options)).to.deep.equal({"0": item5, "60": [item3]});
  });
  ...
  it('places an item (override multiples, half slot)', () => {
    ...
    expect(placeItem({"30": [item1]}, item5, options)).to.deep.equal({"0": item5, "30": [splitItem1]});
  });
